...
# Monaco circuit flag
df_all['IsMonaco'] = df_all['EventName'].str.contains('Monaco', case=False).astype(int)
...
# ── 5. PREDICT Monaco 2026 ─────────────────────────────────────────────────────
print("\n🏁 Building Monaco 2026 predictions...")
...
monaco_input = driver_2026_stats.copy()
monaco_input['IsMonaco'] = 1
monaco_input['GridPosition']     = monaco_input['AvgGrid']
monaco_input['QualiPosition']    = monaco_input['AvgQuali']
monaco_input['RecentQualiForm']  = monaco_input['AvgQuali']
monaco_input['RecentSprintForm'] = monaco_input['AvgSprint']
monaco_input['RaceCraft']        = monaco_input['AvgPositionsGained']
...
monaco_input['DriverEncoded'] = safe_encode(le_driver, monaco_input['FullName'])
monaco_input['TeamEncoded']   = safe_encode(le_team,   monaco_input['TeamName'])

X_monaco = monaco_input[FEATURES].fillna(monaco_input[FEATURES].mean(numeric_only=True))
monaco_input['PredictedPosition'] = model.predict(X_monaco)
monaco_input = monaco_input.sort_values('PredictedPosition').reset_index(drop=True)
monaco_input.index += 1  # 1-based ranking

# ── 6. OUTPUT ─────────────────────────────────────────────────────────────────
print("\n" + "═" * 55)
print("🏆  MONACO GRAND PRIX 2026 — PREDICTED FINISHING ORDER")
print("═" * 55)
print(f"{'Pos':<5} {'Driver':<25} {'Team':<25}")
print("─" * 55)
for pos, row in monaco_input.iterrows():
