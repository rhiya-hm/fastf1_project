...
# ── Cache setup ──────────────────────────────────────────────────────────────
CACHE_DIR = Path(__file__).resolve().parent.parent / 'cache'   # ← replaces fastf1.Cache.enable_cache('cache')
...
# Canada circuit flag
df_all['IsCanada'] = df_all['EventName'].str.contains('Canada', case=False).astype(int)
...
# ── 5. PREDICT Canada 2026 ─────────────────────────────────────────────────────
print("\n🏁 Building Canada 2026 predictions...")
...
canada_input = driver_2026_stats.copy()
canada_input['IsCanada'] = 1
canada_input['GridPosition']     = canada_input['AvgGrid']
canada_input['QualiPosition']    = canada_input['AvgQuali']
canada_input['RecentQualiForm']  = canada_input['AvgQuali']
canada_input['RecentSprintForm'] = canada_input['AvgSprint']
canada_input['RaceCraft']        = canada_input['AvgPositionsGained']
...
canada_input['DriverEncoded'] = safe_encode(le_driver, canada_input['FullName'])
canada_input['TeamEncoded']   = safe_encode(le_team,   canada_input['TeamName'])

X_canada = canada_input[FEATURES].fillna(canada_input[FEATURES].mean(numeric_only=True))
canada_input['PredictedPosition'] = model.predict(X_canada)
canada_input = canada_input.sort_values('PredictedPosition').reset_index(drop=True)
canada_input.index += 1  # 1-based ranking

# ── 6. OUTPUT ─────────────────────────────────────────────────────────────────
print("\n" + "═" * 55)
print("🏆  CANADIAN GRAND PRIX 2026 — PREDICTED FINISHING ORDER")
print("═" * 55)
print(f"{'Pos':<5} {'Driver':<25} {'Team':<25}")
print("─" * 55)
for pos, row in canada_input.iterrows():
